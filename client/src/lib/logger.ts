...
export function logApiRequest(method: string, url: string, userAgent?: string) {
  if (process.env.NODE_ENV === 'development') {
    ...
    console.log(`\x1b[36m[${timestamp}]\x1b[0m \x1b[33m${method}\x1b[0m ${cleanUrl} - \x1b[32m${userAgent?.split(' ')[0] || 'unknown'}\x1b[0m`);
  }
}
...
export function logApiResponse(method: string, url: string, status: number, duration?: number) {
  if (process.env.NODE_ENV === 'development') {
    ...
    console.log(`\x1b[36m[${timestamp}]\x1b[0m \x1b[35m${method}\x1b[0m ${cleanUrl} - ${statusColor}${status}\x1b[0m${duration ? ` \x1b[90m(${duration}ms)\x1b[0m` : ''}`);
  }
}
